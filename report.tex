...
very useful feedback on intermediate presentations: Andreas, Dominique, Frank and Wouter. These friendly people have a lot of experience and made me aware of the actual state of my drafts. I also had a lot of curious friends such as Jonathan, Alexander, Emily, Zhiyu, Michael, Dieter, Ben, Edward, Zhiquan.  They listened to me when I tried to explain to them what I was reading or writing. Sometimes they asked questions 
...
(or constructive) logic \cite{Heyting1930}. The difference between intuitionistic and traditional logic, is the lack of the following propositions which are extra  (unneccesarry) assumptions or axioms according to intuitionistic logic:
...
\subsection{Judgments and contexts}\label{judgments}
...
\item To compute with terms and types, other rules that describe how it is possible to apply terms and types to other terms and types. These rules are 
called \keyword{eliminator}s. The \keyword{computation rule}s determine how an eliminator can act on a constructor. 
\end{itemize}
\end{definition}
...
% 
% There are two proof assistants that have a different approach to dealing with 
% contexts: 
% 
% \begin{itemize}
%  \item Coq \cite{Arias2019} which treats contexts and proofs as interactive meta-objects on which typing rules from \cref{typingrules} can be applied using \keyword{tactics}.
%  \item Agda \cite{Abel2019} treats context dependency and typing rules as a functional dependency which makes Agda less of an (interactive) proof assistant than Coq. There is however a mechanism of substitution of metavariables that is similar.
% \end{itemize}
...
mathematician, see \cref{functionel} and for an introduction to this style \cite{Girard1990}, chapter 2.

% \cref{deductionexample}. 
% 
% \begin{example}\label{deductionexample}
% The proof of the proposition $A \Rightarrow \neg \neg A$ 
...
This paradox is solved in type theory by
...
When the second term $b$ does not depend on the first term  $a$, the dependent sum becomes a finite product which is denoted by $A \times B$. A funite sum type has as terms the tuples of 
...
non-zero in the form of a dependent function.}
\end{figure}
...
of $p$ and $q$.
    \item The intensional identity type supports \keyword{indiscernability of 
...
code formalizing large portions of popular mathematics. The Univalent Foundations project was a joint work undertaken by mathematicians active in the fields of logic, computer science, algebra and topology around consequences of the univalence axiom that resulted in a textbook for univalent type 
theory, also called \keyword{homotopy type theory} \cite{Voevodsky2013}. The practical aspects of this project are discussed in \cite{Bauer2013}. Meanwhile, there are 
more introductions available to univalent type theory such as the short introduction for mathematicians \cite{Escardo2018}, the longer introduction \cite{Grayson2018} and 
the implementation oriented course text \cite{Escardo2019}.
...
\begin{definition}
A \keyword{groupoid} is a category with only invertable morphisms. 
\end{definition}
...
models. The homotopy interpretation of univalent type theory, 
...
More examples of presheaves will arise later in the form of cubical sets (see \cref{cubset}) or can be found in 
...
Every bounded distributive lattice (a partially ordered set with relations) gives rise to a free de Morgan algebra, see \cref{lattice}. This means that the connections can be interpreted as follows: $\wedge$ corresponds to taking the mininum and $\vee$ corresponds to taking the maximum.
...
\begin{definition}[Category of cubes]\label{cubcat}
  The category $\mathcal{C}$ contains as objects the  finite subsets $I \subset 
\mathbb{A}$, called (dimension) names. Morphisms in this category are maps into 
De Morgan algebras. More precisely, given two objects $I,J \in \square$, $f 
\in \text{Hom}_{\square}(J,I)$ if and only if $f: I \rightarrow dM(J)$ is a 
set map.
  \begin{itemize}
  \item  If $f \in \homo{\square}{J}{I}, g \in \homo{\square}{K}{J}$, 
...
From now on, until the rest of this chapter, the symbol $\square$ will be 
used to denote the category defined in \cref{cubcat}. $\square$ can be 
...
\square$, set $\mathbb{F}(I)$ to be the subset of extents in 
$\mathbb{F}$ that only contains dimension names $i \in I$. If $f \in 
\homo{\square}{J}{I}, \mathbb{F}(I) \rightarrow \mathbb{F}(J): \varphi 
...
in the presheaf model over the cube category $\square$ are by \cref{cubset} 
...
category $\square$ to the same set $\singleton$. 
...
$$\int_{\square} 1 = \left\{ (I, \rho ) \mid I \in \square, \rho \in 1I 
\right\} \cong \left\{ (I, \star ) \mid I \in \square \right\}  \cong 
\square$$ where the last isomorphism is just a bijection that works by 
...
$\square \rightarrow \set : I \mapsto dM(I)$.  $(i=0)$ acts as a term 
...
The interval object (or any other context) can also be seen as a type in the empty context $() \vdash \mathbb{I}$. In this interpretation it is a functor  $\int_{\square} \Gamma \rightarrow 
...
The presheaf model over $\square$ also contains the context extensions from 
...
\square,  \rho \in \Gamma (I) \right\} \rightarrow \set: (I, \rho) \mapsto 
\mathbb{F}(I)$$ where $\rho$ is just ignored. Terms $\Gamma \vdash \varphi : 
\mathbb{F}$ are defined by a dependent function $$\varphi : (I : \square) 
...
Take an $I \in \square$ and  $\rho \in (i,j,k: \mathbb{I})$. Then $\rho = 
...
two functors. First it has to be proven that for all $I \in \square$, the 
...
\begin{definition}
Let $\Gamma$ be context and $I \in \square, i \not \in I, \rho \in \Gamma 
...
A generalization of extents, in this text called \keyword{generalized extent}, is given in \cite{Huber2016}, page 95 and can be summerized as follows:
...
over the cube category, types have an intuitive description as cylinders. When $\Gamma \vdash A$ is a type in the presheaf model over cubical 
sets, it is described by a functor from the category of elements 
$$\text{Ty}:\int_{\square} \Gamma \rightarrow \set.$$ This is noted as $A(\rho) \in \set$ for some $|I|$-dimensional hypercube $\rho 
\in \Gamma (I)$. At the same time the substitutions that apply to objects in $\square$ and substitute dimension variables can 
be lifed by \text{Ty} to the level of types. 
...
The most important operation in cubical type theory is the composition operation 
...
The use of the identity type in cubical type theory is replaced by the use of a path type. There is also a naming problem because cubical type theory is modelled with presheaf models and the standard 
model of the identity type is defined as a functor $\op{Id}$: if $\Gamma \vdash A$, $\Gamma \vdash a : A$ and $\Gamma \vdash b : A$, then 
...
Now that the basic types for cubical type theory (omitting the definitions of dependent product and sum types that are given for general presheaf models in \cite{Huber2016}, section 1.2) have been modeled by their semantics in cubical sets, the concepts from \cref{contractible} can be translated to their analogue in cubical type theory. The 
...
% discussed. Code written in Agda can contain expressions or notations that are 
...
equivalences (which is simply a bijection for discrete set-like types or h-sets, see \cref{hset}) between 
...
equality is a term \texttt{algPath} of type \texttt{s₁ ≡ s₂}. The compatibility of the operations means that 
...
complicated properties and algebraic structures. This shows that it can be 
possible to do ``isomorphism-invariant'' algebra in practice.  The 
...
The first time dimension variables or cubes were used in type theory was in \cite{bernardy2012computational}. The theory of \keyword{nominal sets} which is introduced in \cite{Pitts:2013:NSN:2512979} introduced a way of working with dimension variables as objects in a category. In \cite{Bezem2014}, this idea was extended to a categorical model of type theory in cubical sets together with Kan extension properties. The way nominal sets were used in \cite{Bezem2014} was discussed in \cite{Pit14}. Cubical sets were however already discussed in \cite{GraMau03}. The model was extended in \cite{huber2015model} to include more detailed proofs and semantics of the universe. In \cite{Coq13} it was suggested how this model could be allow a proof of the univalence axiom. By that time there was already an implementation of \cite{huber2015model} available in \cite{Moertberg2015}. The proof of univalence with this model was given in \cite{Bezem2018} but slightly more complicated than in later models and higher inductive tpes could not yet be modelled. 
...
higher inductive types. There is still research going on on whether the lattice structure is necessary in the base category and other models such as \cite{Altenkirch2017} do not make used of it.
...
evaluation in \cite{Abe13} can be useful.
...
theory into an example of a theory 
