...
the implementation oriented course texts \cite{Altenkirch2017} and \cite{Escardo2019}.
...
mathematics. The philosophical aspects of the univalence axiom as a foundation for mathematics were discussed in \cite{LadPre16} and more practical aspects for  synthetic mathematics in \cite{Shulman2017}. 
...
different proofs for properties of isomorphic structures as equal. 
...
The goal of this example is to create a term of the path type of cubical type theory (see \cref{pathindprop}) between  the structures $s_1$ 
...
$s_2$. More specifically, if there is a proof of the fact that $s_1$ is commutative, there is also a proof of the fact that $s_2$ is commutative. 
...
\cref{hset}), see for an example of the definition of a mgma \cref{m2def}. 
...
In this section will be explained how a term \texttt{algPath} of the type \texttt{s₁ ≡ s₂} can be defined. Such a term is path with endpoints $s_1$ and $s_2$ that coincides at time $i$ with a ``point-wise 
...
However, the operator \texttt{opᵢ'} from \cref{zeroPathLift} does not have the right 
operators at its ending points however and it is necessary to define another 
operator \texttt{opᵢ} for which \texttt{opᵢ i1 ≡ op₂}, see \cref{adaptedOp}.
...
endLemma i x y =
  (op₂' x y
   ...
   transport fEq (op₁ (transport (sym fEq) x) 
    (transport (sym fEq) y))
       ≡⟨ transpR (op₁  (transport (sym fEq) x) 
            (transport (sym fEq) y)) ⟩
...
This example shows that it is possible to do a version of the ``isomorphism-invariant'' mathematics as claimed in \cite{Voevodsky2013} in practice within the context of simple algebra. 

The requirement for the applicapibility of this example seems to be the construction of a homotopy type equivalence \texttt{f} over which an intermediate operator has to be defined. This homotopy equivalence between carrier types can is a roughly a bijection for discrete set-like types or h-sets as defined in \cref{hset}. The proofs in \cref{adaptedOp} can be read as proofs of the property that the bijection \texttt{f} is a morphism in the category of magmas. However, the proof of being a morphism is harder in cubical type theory than using standard type theory because of the explicit use of transports. Not all of the necessary lemmas about equivalences have been implemented yet. 

It can be concluded that trying to use a (weak form of) the structure invariance principle in cubical type theory is not the easiest approach for 
proving algebraic properties up to isomorphism. However, not all structures in 
...
algebraic computations and proofs. It is also possible that the encoding of natural numbers without zero as a dependent sum was not the best choice in implementations. There are other succesful related implementations such as the implementation of a transport of properties between unary and binary encodings of natural numbers in the file \texttt{Data/BinNat/BinNat.agda} in \cite{Moertberg2018}.
