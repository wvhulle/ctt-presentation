...
\begin{proof}
The number $a = \sqrt{2}$ is irrational by the constructive proof in \cite{Rosenblatt1983}, p. 18 and $b=2\text{log}_2(3)$ is also constructively irrational and by computation $a^b=3$.
\end{proof}
...
\item To compute with terms and types, \keyword{eliminator}s describe how to apply terms and types to other terms and types. For example, the eliminator of the natural numbers takes a property about the natural numbers and states that it is enough to prove this by induction to prove it for all natural numbers. The \keyword{computation rule}s determine how an eliminator can act on a constructor. 
\end{itemize}
\end{definition}
...
mathematical proofs \cite{Abel2019}. Program extraction can be done for 
...
dependent types, new dependent types can be built. See 
...
When $a=_X b$, the terms $a$ and $b$ satisfy the  same properties and it is the intensional identity type can be treated as the jugmental equality from mathematics which explains the notation ``$=$''. 
...
\mapsto p \star q$ such that $\op{refl}_x \star \op{refl}_x =
...
defined the intensional identity type in \Cref{equalitytype}, the previous sections can be summerized in a definition. Because there are many 
...
With equality having been defined and implemented, the definitions of mathematical objects that use equality relations between 
elements of sets can be translated to the language of type theory. An example is the type 
...
\subsection{Univalent foundations of mathematics}\label{univalentfoundations}
...
multiple bijections between these h-sets. Bijections between h-sets 
...
there are also multiple equalities and this is unsatisfactory. An h-set should only be equal to 
...
contains the proofs of equality between proofs of equality. This construction can be contained inductively to obtain a third layer etc. }
\end{figure}
...
Given a category with families, the informal 
description in \Cref{dependencies} becomes a very short definition:
...
set of edges, $\Gamma$ is a \keyword{directed graph}. The set map $\Gamma (f)$ gives the starting vertex of an edge, $\Gamma (g)$ gives the ending vertex. 
\end{example}
...
starting and ending vertex $v$. This means that from every pre-sheaf, a unique \keyword{directed reflexive graph} can be derived. 
\end{example}
...
Examples of contexts for pre-sheaf models can be found in \Cref{pre-sheafexs}. The central example in this text is cubical sets, see \Cref{cubset}. 
...
map $A(f): A(1,e) \rightarrow A(0, \Gamma (f) (e))$.  The 
same applies to the morphism $g$. Let $\Gamma (f) (e) =v$. $f$ can be interpreted as 
...
\item If $I=0$, then an element $\rho \in \Gamma (0)$ is an edge in $\Gamma$. The morphisms $f,g:0\rightarrow 1$ act on terms. Assume for 
example that $f$ (actually $\Gamma(f)$)  is the morphism giving the source of edges. 
...
interesting morphism that acts on $\nu$ is $h$ (or more precisely, the associated set map $\Gamma (h)$). 
...
of nodes and edges is compatible. Taking $\sigma$ as the graph morphism projecting lifted edges $a\rho \mapsto \rho$ and vertices $a\nu \mapsto \nu$, \Cref{depgraphdiag} illustrates  \Cref{typelemma} by showing that each type in the pre-sheaf model as defined in 
...
theory with pre-sheaves has been discussed in \Cref{catsem}. However, remember from \Cref{simpmod} that the goal is to find a constructive model in category theory. Such a model would allow to prove the univalence axiom by generating explicit terms of the univalence axiom. These terms would consist in explicit functions that can help in further computations related to univalent type theory. The model in cubical sets was proven to be a model of univalent type theory, so the univalence axiom holds. When the univalence axiom holds, its function can be explicitly given in terms of a mapping between the primitives of cubical type theory. 
...
Consider a De Morgan algebra generated by dimension 
...
bounding elements $0, 1$. It can also be denoted by $dM(I)$.
\end{definition}
...
Morgan algebra $dM(J)$.
    \item The identity morphism in this category is just the identity set map.
\end{itemize}
    \end{definition}
...
interpreted as an abstract higher-dimensional cube on which operations described by a De Morgan algebra can be applied. As a base category it has 
...
If $I = \left\{x,y \right\}$,  $u \in \Gamma(I)$ has an interpretation as 
...
cubical set. For example, if $p(x,y,z) = 1 + x^2y +z \in R[x,y,z]$, then the application of the face map $(y/0)$ gives $(1+x^2 y +z ) (y/0) = 1+z$.
\end{example}
...
\mathbb{A}, b \in \left\{ 0,1 \right\}$). These substitutions are also written as $i = b$ (variations of cubical type theory also allow other values for $b$, see \Cref{compcubes}). An element of the lattice $\varphi 
...
On the other hand, $\mathbb{F}$ can also be seen as a type in $\psh{\cube}$, see \Cref{facetype}. 

Remember from \Cref{premodel} and \Cref{precon} that in the presheaf model $\psh{\cube}$, contexts are 
pre-sheaves and types are pre-sheaves over a category of elements. The contexts 
in $\psh{\cube}$ are by \Cref{cubset} called cubical sets.
...
model $\psh{\cube}$, it is necessary to describe how $\mathbb{F}$ acts on the category of elements. By definition of the category of elements (see \Cref{catel}), 
...
Traditional types that are defined in 
(univalent) type theory can be interpreted in $\psh{\cube}$. However, the face and interval objects form a more important part of cubical 
type theory because they can be used to index and restrict arbitrary contexts. 
...
\item When $\varphi$ is an extent containing only variables $i,j,k \in \mathbb{A}$ (for example $\varphi = (i = 0) \wedge (j = 1) \vee (k = 0)$), it is a term $(i: \mathbb{I}), (j: \mathbb{I}), (k: \mathbb{I}) \vdash 
    ...
    \begin{itemize}
    \item  The set $\rho$ can be rewritten with the definition of a context extension (see \Cref{contextindexing}) as $\rho = ((\rho_i, \rho_j), \rho_k)$, where $\rho_i \in dM(I)$ and $\rho_j \in \mathbb{F}(I,\rho_i)$ which means that $\rho_j$ is an extent that only uses the     variables in $I$. 
    \item Furthermore, by \Cref{pretm} and the above, $\rho_k \in \mathbb{F}(I, (\rho_i, \rho_j))$. Define $\varphi(I,\rho)=\varphi(I,(\rho_i,\rho_j,\rho_k))$ as 
    the extent where every variable $i$ occurring in $\varphi$ is replaced by the 
    corresponding $\rho_i$ and apply the rules in \Cref{facecubical}.
    \end{itemize}
...
(j=1)$. So if the attention is restricted to these sides, it is clear that $a$ lies in the restriction of the original context: $i , j : \mathbb{I}, 
...
% alternative definition lets the choice of $u$ depend on $\rho \in \Gamma (I)$. 
% 
% \begin{definition}
...
For each $\Gamma \vdash u : A$ and extent $\varphi$, there is a partial element 
...
other words $v = u{\iota}_{\varphi}$, the term $v$ is called \keyword{extensible}. This can be intuitively seen as the term $v$ being extendible to $u$ by dropping the 
...
This term also has to satisfy the elimination rules mentioned in \cite{Huber2016}, figure 6.4.  A system allows to treat compatible terms, terms that agree at all points of the hypercube, as a single term. \Cref{extentrewrite} states that certain compositions can be treated as generalized extents.
...
cylinder similarly to the action of the morphisms $f$ and $g$ in \Cref{depgraphdiag}. This is visualized in \Cref{hubtypes}.  Another way to interpret the type $A$ is using \Cref{typelemma} by viewing the type $A$ as a new context $\Delta$.  
\end{example}
...
\par\end{centering}
\caption{\label{fig:Kan}Daniel Kan (1927-2013) was a Dutch category and homotopy theorist. He defined his Kan extension property  in the context of algebraic topology for cubical sets, see his book \cite{Kan1955} saying how boxes can be filled. \cite{Coquand2019} mentions that there is a predecessor of this property in \cite{Eilenberg1939}. This Kan extension property later evolved into different flavours such as a version for simplicial sets stating that every ``horn'' can be filled and the composition operation of cubical type theory, see \Cref{compdef}. Picture taken in his home in 
...
words, the ``lid'' of the ``open'' box $\rho$ can be closed along the side $i=1$.
...
Again, this operation can be written as aa introduction rule telling the 
...
Using the definition of extents, the filling term can be decomposed
...
The filling is the composition plus something else, also called the interior, but the interior in this context is not the interior in a topological sense 
because there is no concept of continuity or topology present in $\psh{\cube}$. In $\psh{\cube}$ it is only possible to compute with a finite amount of (subsets of) corners of 
...
\rightarrow P \ A\ A \ (\texttt{idEquiv} \ A)$, then there is
a proof of $P$ any $A \ B: \mathcal{U}$: $$ P\  A \ B\ e.$$
\end{theorem}
...
\begin{example}
Let $A = \op{Bool}$ and $B = \op{Fin}\ 2$, then $A = 
B$. This holds because every bijection between $A$ and $B$ which is also an 
...
isomorphism, an equivalence can be extracted. Applying $\op{isoToIsEquiv}$ 
...
In combination with the standard library of Agda \cite{Danielsson2019}, concrete examples in univalent type theory of reasoning 
with univalence can be produced.
...
add information. With the univalence axiom, different proofs for properties of isomorphic structures are equal. In \cite{Danielsson2012} this was proven already but not yet for a constructive interpretation of the univalence axiom.
...
This example is mentioned in the introduction of \cite{Coquand2013}, but it has not been verified if their specific encoding of isomorphisms makes transport of properties work.
...
\end{BVerbatim}
\caption{The definition of an intermediate operator. Here, \texttt{zeroPath} is a function used to return arguments to the space $\mathbb{N}$ defined with the help of a connection $\wedge$.}
...
This example shows that ``isomorphism-invariant'' mathematics is (partially) possible as claimed in \cite{Voevodsky2013} in practice with magmas using a constructive interpretation of the univalence axiom. 
...
of its contents. The resulting datatype is then called generic, the approach 
...
\section{Other applications}\label{futapp}
...
from univalent type theory are defined in part two of  \cite{Voevodsky2013}. These chapters only use higher 
inductive types which are already implemented in cubical type theory and some applications of univalence. So more advanced concepts can also be formalized in cubical type theory. Concepts from univalent type theory that can be useful and 
...
fundamental group of $S^3$, but computations of other 
homotopy groups of topological spaces could be more efficient when \cite{HoBr17} is 
combined with an interface to cubical type theory, for example through 
\cite{Moertberg2018}.


\item Next to homotopy groups, there are homology groups of topological 
spaces as well. For a fixed topological space, these do not necessarily have to be the 
...
type theory such as \cite{GaSeSi99}. The text 
\cite{Graham2018} uses type theory to  improve on this. Type theory may be be better suited for solving 
...
\cite{Harper1991}. A historical overview of the relation between computational type theory and \Cref{typetheory} is given in \cite{Constable2003} and \cite{Constable2015July}. A computational type theory is usually built as 
follows:

\begin{itemize}
 \item First, there is a definition of a set of ``closed terms'' which are like 
the terms of the intensional type theories, but they are not typed, in the sense 
...
theory is not explicitly mentioned because dimension variables are not considered 
as independent objects. This implies that the theory is not formally proven to be consistent 
...
interval ($0$ or $1$) or a dimension name. This alloes substitutions of 
the form $(x/y)$ (or in alternative notation $x = y$) that can be considered as diagonals in a square. This can also 
...
Cartesian cubical computational type theory was implemented in Haskell \cite{MorAng18} and in the language RedPRL \cite{Sterling2018a} with a proof of univalence that is slightly longer than the one in \cite{Moertberg2018}. Development was continued on a successor called RedTT 
\cite{Sterling2018b} which has many similarities but also extra features such as 
extension types according to \cite{Moertberg2018a}. Development on RedTT stopped by the end of 
2018, but the library code in \cite{Sterling2018b} still contains an alternative 
proof of univalence. The syntax of RedTT is different from \cite{Moertberg2018}, but the 
structure of the libraries is very similar. The composition and composition 
operations used in \cite{Sterling2018b} have also been implemented in Agda Cubical in
...
finite 
amount of applications of the constructors of its type. Canonicity does not 
...
Given a term $t: \mathbb{N}$ constructed using the univalence axiom, there are two terms $u : \mathbb{N}$ and $p : t =_{\mathbb{N}} u$ (that can 
involve univalence) such that $u$ 
does not involve the univalence axiom.
\end{conjecture}

This conjecture can fail to hold when axioms such as function extensionality (see \Cref{extensionality}) or 
univalence are added to the type theory. The conjecture was not solved for 
...
set model and cubical type theory (see \Cref{cubical}) which is an extension of univalent type theory. With the help of the \texttt{Glue} construction that is part of the cubical set model (see \Cref{glueOp}), there is a proof the univalence axiom in cubical type theory. This was discussed in \Cref{univalenceproof}. The application of the 
...
theory can give a topological intuition about using the structure 
...
type theory using the primitives of the cubical set model which are in essence
cubes together with a composition and glueing operation. 

However, it is still an open challenge to  prove normalizability and 
use canonicity in practice for formalizing fundamental theorems coming from homotopy theory, see 
\Cref{normalizing}. There also seem to be problems with 
...
Univalent foundations of mathematics (see for future applications \Cref{univalentfoundations} and \Cref{futapp}) might be useful in the future, but without computational interpretation, not every proof that uses univalence can be verified, or at least not by computation. The implementation of the primitives in cubical type theory will have to be optimized to increase the overall efficiency of the normalization process. 
